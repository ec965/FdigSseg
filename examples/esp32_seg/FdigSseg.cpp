...
void FdigSseg::init(uint8_t hw_timer, uint32_t division, uint32_t frequency){
    // largest string is 9 chars:
    // '0.0.0.0.\0'
    queue = xQueueCreate(10, 9*sizeof(char));
    Serial.println("made q");
    timer = timerBegin(hw_timer, division, true);
    timerAttachInterrupt(timer, &FdigSseg::startCallbackImpl, true);
    timerAlarmWrite(timer, frequency, true);
    timerAlarmEnable(timer);
    Serial.println("finished timer setup");

    xTaskCreate(&FdigSseg::startTaskImpl, "7segment display", 4000, this, tskIDLE_PRIORITY, &handle);
    Serial.println("task made");
}

void IRAM_ATTR FdigSseg::callback(){
    xTaskResumeFromISR(handle);
}

void FdigSseg::startCallbackImpl(){
    FdigSseg* seg;
    seg->callback();
}

void FdigSseg::startTaskImpl(void* parameter){
    FdigSseg* seg = reinterpret_cast<FdigSseg*>(parameter);
    seg->task();
}
...
void FdigSseg::task(){
    ...
    while(1){
        vTaskSuspend(handle);
        Serial.println("got isr");
        xQueueReceive(queue, &str, 0);
...
void FdigSseg::display_switch(char* str){
    switch (digit_inc){
        case 0:
            if (str[1] == '.'){
...
void FdigSseg::display(char* str){
